...
class Contrjeu{
        ...
        if(!isset($_SESSION['selectionne']) || ($_SESSION['selectionne']['x'] == $x && $_SESSION['selectionne']['y'] == $y )){
            ...
            if ($_SESSION['selectionne']['y'] == $y || $_SESSION['selectionne']['x'] == $x) {
                ...
                if($this->sontLiablesHorizontalement($x, $y)){
                    ...
                    //ne sont pas liables
                    //TODO: à remplir car actuellement pas de cas comme cela
                    //TODO: c'est quand il y a une ville (ou un pont) entre les deux villes
                    return;
                }
    ...
    public function sontLiablesVerticalement($x, $y){
        ...
        while ($sontLiables && $i <= $maxy){
            if($this->modeleVilles->existe($x, $i)){
                $sontLiables = false;
            }
            $i++;
        }
    ...
    public function sontLiablesHorizontalement($x, $y){
        ...
        while ($sontLiables && $i <= $maxx){
            if($this->modeleVilles->existe($i, $y)){
                $sontLiables = false;
            }
            $i++;
        }
